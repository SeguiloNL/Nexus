...
export default async function ActivationsPage() {
  ...
  return (
    <ActivationOrderList
      ...
      canCreate={canUserRole(session.user.role, "create", "activationOrder")}
      canEdit={canUserRole(session.user.role, "edit", "activationOrder")}
      canDelete={canUserRole(session.user.role, "delete", "activationOrder")}
    />
  );
}
