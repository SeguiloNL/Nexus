...
import {
  commitTrackerCsvAction,
  previewTrackerCsvAction,
  type CsvImportActionState,
} from "../../actions";
import { formatImei } from "@/lib/formatters";

export function TrackerImportClient() {
  const router = useRouter();
  const [state, formAction] = useFormState<CsvImportActionState>(
    previewTrackerCsvAction as any,
    {}
  );

  const [commitState, commitAction] = useFormState<CsvImportActionState>(
    commitTrackerCsvAction as any,
    state
  );
  ...
  useEffect(() => {
    if (commitState.committed) {
      toast.success(
        `${commitState.committed.count} trackers succesvol geïmporteerd.`
      );
      setTimeout(() => router.push("/trackers"), 1200);
    }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Upload className="h-5 w-5" /> Trackers importeren (CSV)
          </CardTitle>
          <CardDescription>
            Upload een CSV-bestand met kolommen: <code className="bg-slate-100 px-1 rounded">serialNumber</code>, <code className="bg-slate-100 px-1 rounded">imei</code>, <code className="bg-slate-100 px-1 rounded">brand</code>, <code className="bg-slate-100 px-1 rounded">model</code> (optioneel: hardwareType, firmwareVersion, purchaseDate, supplier, notes).
          </CardDescription>
        </CardHeader>
        <CardContent>
          {!preview ? (
            ...
            <div className="space-y-4">
                      ...
                      <table className="w-full text-xs">
                        ...
                        <tbody>
                          {preview.valid.slice(0, 20).map((r: any) => (
                            <tr key={r.row} className="border-t">
                              <td className="px-3 py-2 font-mono">{r.row}</td>
                              <td className="px-3 py-2 font-medium">{r.data.serialNumber}</td>
                              <td className="px-3 py-2 font-mono">{formatImei(r.data.imei)}</td>
                              <td className="px-3 py-2">{r.data.brand} {r.data.model}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
              ...
              {committed ? (
                <div className="rounded-md border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 flex items-center gap-2">
                  ...
                  <strong>{committed.count}</strong> trackers toegevoegd. Je wordt zo terug gestuurd...
                </div>
              ) : null}
                  ...
                  >
                    {committed ? (
                      ...
                      <>
                        ...
                        {preview.valid.length} trackers importeren
                      </>
                    )}
                  </Button>
                </div>
